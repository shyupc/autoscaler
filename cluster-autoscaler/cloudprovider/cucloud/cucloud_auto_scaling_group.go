...
import (
	...
	cucloudsdkv2model "k8s.io/autoscaler/cluster-autoscaler/cloudprovider/cucloud/cucloud-sdk-v2/services/v2/model"
	"k8s.io/autoscaler/cluster-autoscaler/config"
	"k8s.io/klog/v2"
	schedulerframework "k8s.io/kubernetes/pkg/scheduler/framework"
)

// AutoScalingGroup represents a CuCloud's 'Auto Scaling Group' which also can be treated as a node group.
...
func newAutoScalingGroup(manager CucloudManager, sg cucloudsdkv2model.ScalingGroup) AutoScalingGroup {
	return AutoScalingGroup{
		manager:           manager,
		groupName:         sg.Name,
		groupID:           strconv.Itoa(sg.ID),
		minInstanceNumber: sg.AutoScale.NodeMin,
		maxInstanceNumber: sg.AutoScale.NodeMax,
	}
}
...
func (asg *AutoScalingGroup) TargetSize() (int, error) {
	desiredInstanceSize, err := asg.manager.GetDesiredAsgInstanceSize(asg.groupID)
	if err != nil {
...
func (asg *AutoScalingGroup) IncreaseSize(delta int) error {
	...
	size, err := asg.manager.GetDesiredAsgInstanceSize(asg.groupID)
	if err != nil {
	...
	// do increase instance
	err = asg.manager.IncreaseAsgInstanceSize(asg.groupID, delta, asg.ruleName)
	if err != nil {
...
func (asg *AutoScalingGroup) DeleteNodes(nodes []*apiv1.Node) error {
	...
	// delete node from node group and k8s cluster
	err = asg.manager.DeleteInstances(asg.groupID, instanceIds, asg.ruleName)
	if err != nil {
